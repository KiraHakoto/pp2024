...
class Student(Person):
    ...
    def calculate_gpa(self):
        if not self.marks or not self.credits:
            return 0
        return np.average(self.marks, weights=self.credits)
...
class School:
    ...
    def input_marks(self):
        ...
        for i, student in enumerate(self.students):
            ...
            print(f"Rounded mark for student {i+1} ({student.name}) in course {course_id}: {mark}")
            student.marks.append(mark)
            student.credits.append(course.credits)
